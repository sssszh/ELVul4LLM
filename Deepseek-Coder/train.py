...
from Dataset.DeepseekCoderDefectDataset import DeepseekCoderBaseDataset
...
def run_training(args, train_data, val_data):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(model_path, quantization_config=q_config, device_map={"": Accelerator().process_index}, trust_remote_code=True, config=config)
...
def get_dataset(args, mode="train"): 
    ...
    train_data = DeepseekCoderBaseDataset(
        dataroot=dataroot,
        problems=problems_1,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_labels', default=44, type=int, help="")
    # Training
    ...
    parser.add_argument('--save-freq', default=431, type=int, help='save model checkpoints after this number of training steps')
    parser.add_argument('--save_total_limit', default=15, type=int, help='total of number checkpoints to keep; only keep the latest ones') 
