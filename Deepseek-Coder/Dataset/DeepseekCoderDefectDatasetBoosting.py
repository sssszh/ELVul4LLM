...
class DeepseekCoderBaseDataset(torch.utils.data.Dataset):
    def __init__(self, dataroot, problems, model, max_tokens) -> None:
        ...
        self.tokenizer = transformers.AutoTokenizer.from_pretrained(model, trust_remote_code=True)
    ...
    def initialize(self):
        ...
        for idx, line in tqdm(enumerate(self.problems), ncols=0, total=len(self.problems)):
            ...
            sample_weight = json_line["weight"]
            sample = (code, target, sample_weight)
    ...
    def pack_samples(self, idx):
        ...
        code, target, sample_weight = sample_pool[idx]
        ...
        out_sample = {
            ...
            "sample_weights": torch.tensor(sample_weight)
        }
